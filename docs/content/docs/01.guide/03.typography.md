...
seo:
  ...
  description: Review the current typography roles and happydesigns hierarchy, labels and code guidance.
---
...
::id-brand-reference{section="typography"}
::

## Primary typeface

The primary sans carries the warmth and character of the brand. It should make happydesigns feel design-led and approachable without becoming playful for its own sake.
...
The monospace role is the technical counterweight. It helps code, tokens, versions, and compact metadata scan as implementation detail.
...
```ts [app.config.ts — consumer override]

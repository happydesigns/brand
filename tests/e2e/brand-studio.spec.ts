...
async function setCompare(page: import('@playwright/test').Page, checked: boolean) {
  const desktop = page.getByRole('button', { name: 'Compare original brand', exact: true })
  if (await desktop.isVisible()) {
    if ((await desktop.getAttribute('aria-pressed') === 'true') !== checked) await desktop.click()
    return
  }
  await page.getByRole('button', { name: 'View', exact: true }).click()
  await page.getByRole('checkbox', { name: 'Compare original brand', exact: true }).setChecked(checked)
...
test('viewport presets preserve CSS dimensions, rotate and share custom sizes', async ({ page }) => {
  ...
  const beforeCancel = await viewport()
  ...
  await expect.poll(async () => (await viewport()).height).toBe(beforeCancel.height - 10)
  const cornerBox = await corner.boundingBox()
  const beforeDrag = await viewport()
  await page.mouse.move(cornerBox!.x + 10, cornerBox!.y + 10)
  await page.mouse.down()
  await page.mouse.move(cornerBox!.x - 20, cornerBox!.y - 20, { steps: 4 })
  await expect.poll(async () => (await viewport()).height).toBeLessThan(beforeDrag.height)
  await page.keyboard.press('Escape')
  await page.mouse.up()
  await expect.poll(viewport).toEqual(beforeDrag)
  const heightHandle = page.locator('.studio-frame-wrap').last().getByRole('button', { name: 'Resize viewport height', exact: true })
  const heightBox = await heightHandle.boundingBox()
  await page.mouse.move(heightBox!.x + heightBox!.width / 2, heightBox!.y + 10)
  await page.mouse.down()
  await page.mouse.move(heightBox!.x + heightBox!.width / 2, heightBox!.y - 30, { steps: 4 })
  const duringDrag = await viewport()
  await page.mouse.up()
  await expect.poll(viewport).toEqual(duringDrag)
  await expect(page.locator('iframe[title="Original brand preview"]')).toHaveCSS('transform', await page.locator('iframe[title="Draft brand preview"]').evaluate(frame => getComputedStyle(frame).transform))
  await choose(page, 'Preview width', 'Desktop')
  await page.getByRole('button', { name: 'View', exact: true }).click()
  ...
  await expect(draft(page).getByRole('heading', { name: 'Component examples' })).toBeVisible()
  await expect.poll(viewport).toEqual({ width: 820, height: 1440 })
  await choose(page, 'Preview width', 'Tablet')
...
test('projects survive switching and export preserves the comparison baseline', async ({ page }) => {
  ...
  await expect(page.locator('.studio-project-name').filter({ hasText: 'Project A' })).toBeVisible()
  await page.getByRole('button', { name: 'Brand picker', exact: true }).click()
  await expect(page.getByRole('option', { name: /^Project B(?: |$)/ })).toBeVisible()
  await page.keyboard.press('Escape')
  await projectAction(page, 'Reset draft')
  await expect(page.locator('.studio-project-name').filter({ hasText: source.theme.label })).toBeVisible()
})
...
test('brand picker isolates brands, preserves drafts and protects the Nuxt UI baseline', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Brand picker', exact: true }).click()
  await page.getByRole('option', { name: /^Nuxt UI(?: |$)/ }).click()
  ...
  await page.getByRole('option', { name: /^Studio library draft(?: |$)/ }).click()
  await openPanel(page, 'Brand')
  ...
  await expect(page.getByRole('button', { name: 'Brand picker' })).toHaveText('Nuxt UI')
  await page.getByRole('button', { name: 'Brand picker', exact: true }).click()
  await page.getByRole('option', { name: /^Independent brand(?: |$)/ }).click()
  await openPanel(page, 'Brand')

...
async function openPanel(page: import('@playwright/test').Page, panel: string) {
  if (await page.getByRole('button', { name: panel, exact: true }).isVisible()) await page.getByRole('button', { name: panel, exact: true }).click()
  else {
    if (!await page.getByLabel('Brand settings').isVisible()) await page.getByRole('button', { name: 'Customize', exact: true }).click()
    await choose(page, 'Settings section', panel)
  }
}
async function setCompare(page: import('@playwright/test').Page, checked: boolean) {
  const checkbox = page.getByRole('checkbox', { name: 'Compare original', exact: true })
  const mobile = !await checkbox.isVisible()
  if (mobile) await page.getByRole('button', { name: 'Preview', exact: true }).click()
  await page.getByRole('checkbox', { name: 'Compare original', exact: true }).setChecked(checked)
...
test('projects survive switching and export preserves the comparison baseline', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Download source' }).click()
  await downloading
  await expect(page.getByText(/This version was downloaded/)).toBeVisible()
  await page.keyboard.press('Escape')
  await page.getByRole('button', { name: 'New brand', exact: true }).click()
...
test('view URLs restore safe navigation state without including draft data', async ({ page }) => {
  ...
  await choose(page, 'Template page', 'Home')
